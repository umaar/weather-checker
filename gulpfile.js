...
const paths = {
	...
	copyFonts: {
		input: ["src/client/font/*.woff", "src/client/font/*.woff2"],
		output: `${clientDist}/font`,
	},
	copyViews: {
		input: ["src/server/views/**/*"],
...
function watchFiles(done) {
	gulp.watch(
		paths.copyViews.input,
		gulp.series(["clean:views", "copy:views", triggerChange])
	);
	...
	gulp.watch(
		"src/client/js/**/*.js",
		gulp.series(["clean:js", "scripts", triggerChange])
	);

	nodemon({
		script: paths.server,
		ignore: ["src/", "dist/client/", "node_modules"],
		delay: "100",
		watch: ["dist/server/"],
		ext: "js",
		env: {
